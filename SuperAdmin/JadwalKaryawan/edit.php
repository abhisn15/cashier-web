...
// Get employee schedule data if editing
$id_jadwal = isset($_GET['id']) ? $_GET['id'] : '';
$jadwalData = getJadwalKaryawanById($id_jadwal);

// mengecek tombol submit sudah ditekan atau belum
if (isset($_POST["submit"])) {
  if (editJadwalKaryawanById($_POST) > 0) {
    echo "
        <script>
            alert('Jadwal karyawan berhasil diubah!');
    ...
    echo "
        <script>
            alert('Jadwal karyawan gagal diubah!');
            document.location.href = '../JadwalKaryawan.php';
        </script>
        ";
  }
...
<head>
  ...
  <title>SuperAdmin | Edit Jadwal Karyawan</title>
...
<body>
  ...
  <div class="sm:pl-8 py-5 mx-10 sm:ml-64 sm:mr-10">
    <h1 class="text-2xl">Edit Jadwal Karyawan</h1>
    <br>
    <div class="text-xl flex flex-row items-center gap-4">
      <a href="../JadwalKaryawan.php" class="text-gray-500">Jadwal Karyawan</a>
      <span class="text-gray-500">/</span>
      <a href="#" class="text-orange-500">Edit Jadwal Karyawan</a>
    </div>

    <!-- Form untuk mengedit jadwal karyawan -->
          ...
          <div class="bg-white rounded shadow-lg p-4 px-4 md:p-8 mb-6">
            <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 lg:grid-cols-3">
              <div class="text-gray-600">
                <p class="font-medium text-lg">Edit Jadwal Karyawan</p>
                <p>Tolong periksa dan edit form berikut!</p>
              </div>
                ...
                <form method="POST" action="">
                  <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-5">
                    ...
                    <div class="md:col-span-5">
                      ...
                      <select name="id_karyawan" id="id_karyawan" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" required>
                        <?php foreach ($karyawanData as $karyawan) : ?>
                          <option value="<?= $karyawan['id']; ?>" <?= $jadwalData['id_karyawan'] == $karyawan['id'] ? 'selected' : ''; ?>>
                            <?= $karyawan['nama']; ?>
                          </option>
                        <?php endforeach; ?>
                      </select>
                    ...
                    <div class="md:col-span-2">
                      ...
                      <input type="text" name="hari" value="<?= $jadwalData['hari']; ?>" id="hari" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" required>
                    </div>

                    <div class="md:col-span-2">
                      <label for="jam_mulai">Jam Mulai</label>
                      <input type="time" name="jam_mulai" value="<?= $jadwalData['jam_mulai']; ?>" id="jam_mulai" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" required>
                    </div>

                    <div class="md:col-span-2">
                      <label for="jam_selesai">Jam Selesai</label>
                      <input type="time" name="jam_selesai" value="<?= $jadwalData['jam_selesai']; ?>" id="jam_selesai" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" required>
                    </div>

                    <div class="md:col-span-5">
                      <button class="py-2 px-4 text-sm font-medium text-white bg-blue-500 rounded-lg w-full md:w-auto">Simpan</button>
                    </div>
                  </div>
                </form>
              </div>
