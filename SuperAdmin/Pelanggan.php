...
{
  global $conn;
  $stmt = $conn->prepare("SELECT * FROM users WHERE nama LIKE ? AND role IN ('User')");
  $search = "%$keyword%";
...
if (isset($_POST["cari"])) {
  ...
  if (empty($keyword)) {
    ...
    $total = query("SELECT COUNT(*) AS total FROM users WHERE role IN ('User')")[0]['total'];
  } else {
...
<head>
  ...
  <title>SuperAdmin | Pelanggan</title>
...
<body>
  ...
  <aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-[90dvh] transition-transform -translate-x-full bg-white rounded-r-2xl shadow-xl sm:translate-x-0 border-r-2" aria-label="Sidebar">
    ...
    <div class="h-full px-3 pb-4 overflow-y-auto bg-white rounded-b-2xl bg-gray-800 border-r-2">
      <ul class="space-y-2 font-medium">
        ...
        <li>
          <a href="Produk.php" class="flex items-center p-2 rounded-lg text-grayy-900 hover:text-white hover:bg-orange-400 group">
        ...
        <li>
          ...
          <ul class=" py-2 space-y-2">
            <li>
              <a href="#" class="flex items-center w-full p-2 transition duration-75 rounded-lg pl-11 group bg-orange-400 text-white">Pelanggan</a>
            </li>
            <li>
  ...
  <div class="sm:pl-8 py-5 mx-10 sm:ml-64 sm:mr-10">
    ...
    <div class="text-xl flex flex-row items-center gap-4">
      ...
      <a href="#" class="text-orange-500">Pelanggan</a>
    </div>
    <br>
        ...
        <?php foreach ($pelanggan as $row) : ?>
          <tbody>
            <tr class="bg-white border-b hover:bg-gray-50">
              ...
              <td class="px-6 py-4 text-center">
                <a href="Pelanggan/edit.php?id=<?= htmlspecialchars($row['id']) ?>" class="font-medium text-blue-600 hover:underline">Edit</a>
                <span>|</span>
                <a onclick="return confirm('Apakah kamu yakin ingin menghapus user ini?');" href="Pelanggan/hapus.php?id=<?= htmlspecialchars($row['id']) ?>" class="font-medium text-red-600 hover:underline">Hapus</a>
              </td>
            </tr>
