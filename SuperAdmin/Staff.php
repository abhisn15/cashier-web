...
{
  global $conn;
  $stmt = $conn->prepare("SELECT * FROM users WHERE nama LIKE ? AND role IN ('Staff')");
  $search = "%$keyword%";
...
if (isset($_POST["cari"])) {
  ...
  if (empty($keyword)) {
    ...
    $staff = query("SELECT * FROM users WHERE role IN ('Staff')");
    $total = query("SELECT COUNT(*) AS total FROM users WHERE role IN ('Staff')")[0]['total'];
  } else {
      ...
      <ul class="space-y-2 font-medium">
        ...
        <li>
          ...
          <ul class=" py-2 space-y-2">
            ...
            <li>
              <a href="#" class="flex items-center w-full p-2 transition duration-75 rounded-lg pl-11 group bg-orange-400 text-white">Staff</a>
            </li>
          </ul>
        </li>
        <li>
